...
public class Debat {
    ...
    private final ArrayList<ArrayList<Integer>> allSolutions;

    private final ArrayList<Argument> listeArguments;
    private Path p;
    private final ArrayList <ArrayList<Integer>> solutionsPreferees ;
    ...
    public int getIdFromName(String s) {
        ...
        for (int i = 0; i < listeArguments.size() && !trouve; i++) {
    ...
    public boolean estPasAdmissible(ArrayList<Integer> sol) {
        ...
        if (sol.size() == 1) {
            ...
            for (int i = 0; i < sol.size() && !contradiction; i++) {
            ...
            while ((s = br.readLine()) != null) {
                ...
                if (s.length()>=8 ) {
                    if (s.substring(0, 8).equalsIgnoreCase("argument")) {
                        ...
                        addArgument(a);
                    } else if (s.startsWith("contradiction")) {
    ...
    public boolean verifPref(ArrayList<Integer> sol) {
        ...
        if (estPasAdmissible2(sol)) {
            System.out.println("Votre solution n'est ni admissible ni préférée");
            return false;
        }
        for (int i = 0; i<allSolutions.size(); i++){
            if (!estPasAdmissible2(allSolutions.get(i)) && estContenu(sol, allSolutions.get(i))) {
    ...
    public void setSolutionsPreferees(){
        for (int i = 0; i<allSolutions.size(); i++){
            if (verifPref2(allSolutions.get(i)))
